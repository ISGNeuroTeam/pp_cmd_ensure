...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"ensure_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f ensure-*.tar.gz
	echo Create archive \"ensure-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../ensure-$(VERSION)-$(BRANCH).tar.gz ensure
...
clean_pack:
	...
	cd make_build; tar czf ../ensure.tar.gz ensure && rm -rf ../make_build
