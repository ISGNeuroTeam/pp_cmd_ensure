...
all:
 ...
 test - run all tests\n\
 pack - make output archive, file name format \"pp_cmd_ensure_vX.Y.Z_BRANCHNAME.tar.gz\"\n\
"
...
pack: make_build
	rm -f pp_cmd_ensure-*.tar.gz
	echo Create archive \"pp_cmd_ensure-$(VERSION)-$(BRANCH).tar.gz\"
	cd make_build; tar czf ../pp_cmd_ensure-$(VERSION)-$(BRANCH).tar.gz ensure

clean_pack:
	rm -f pp_cmd_ensure-*.tar.gz


pp_cmd_ensure.tar.gz: build
	cd make_build; tar czf ../pp_cmd_ensure.tar.gz ensure && rm -rf ../make_build
...
make_build:
	...
	echo make_build
	mkdir make_build
	cp -R ./ensure make_build
	cp *.md make_build/ensure/
...
dev: venv
	rm -rf ./venv/lib/python3.9/site-packages/postprocessing_sdk/pp_cmd/ensure
...
test:
	docker build --progress=plain -t pp_cmd_ensure:$(VERSION) . && docker run -v $$(pwd):/app -it pp_cmd_ensure:$(VERSION) python -m unittest tests/test_command.py
